...
def dft(img, P=20) -> Tuple:
    """
    Извлечение признаков методом DFT
...
def dct(img, P=20) -> Tuple:
    """
    Извлечение признаков методом DCT
    (Двумерное дискретное косинус-преобразование)
    """
    M, N = img.shape
    X = img.copy().astype(np.int32)
    t = lambda S, i, j: math.sqrt(2 / S) \
        * math.cos(math.pi * (2 * j + 1) * i / (2 * S))
    T_P_M = np.array(
        [
