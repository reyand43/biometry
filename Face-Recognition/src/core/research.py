...
@templ_num_plot
def research(db_name: str, method: str, temp_to: int) -> Tuple[List, List, List]:
    print(f"{db_name=} ; {method=}")
    images = load("ORL")
    bad_images = load(db_name)
    ...
    for param in range(*range_params):
        ...
        X_train, X_test, y_train, y_test = \
            split_data(data=images, bad_data=bad_images, templ_to=temp_to)
...
def parallel_system_research(
    ...
    for img_num in range(1, images_per_group_num):
        ...
        X_train, X_test, y_train, y_test = \
            split_data(data=images, bad_data=bad_images, templ_to=img_num)
        predicted_tests = []
